...
def run_mirror(pair_id):
    ...
    conn.on("GatewayUserAccount", lambda d: log_msg(pair_id, f"Account: {str(d)[:80]}"))
    # Try alternative event names
    conn.on("GatewayUserAccountOrder", on_order)
    conn.on("GatewayUserAccountPosition", on_position)
    conn.on("GatewayUserAccountTrade", lambda d: log_msg(pair_id, f"AccTrade: {str(d)[:150]}"))
    conn.on("Order", lambda d: log_msg(pair_id, f"Order2: {str(d)[:150]}"))
    conn.on("Position", lambda d: log_msg(pair_id, f"Position2: {str(d)[:150]}"))
    conn.on("Trade", lambda d: log_msg(pair_id, f"Trade2: {str(d)[:150]}"))
    conn.on_open(lambda: [
