...
def run_mirror(pair_id):
    ...
    def on_order(data):
        try:
            ...
            log_msg(pair_id, f"Order: {status} {side} {qty}x {contract}")

            if status == "Filled" and side in ("Buy", "Sell"):
    ...
    def on_position(data):
        try:
            ...
            status   = e.get("status", "")
            contract = e.get("contractId", "")
            pos_id   = str(e.get("positionId", ""))

            log_msg(pair_id, f"Position: {status} {contract}")

            if status in ("Closed", "Liquidated"):
                close_hedge(pair_id, pos_id or contract)

        except Exception as ex:
            log_msg(pair_id, f"Position event error: {ex}")

    conn.on("GatewayUserAccountOrderEvent", on_order)
    conn.on("GatewayUserAccountPositionEvent", on_position)
    conn.on("GatewayUserAccount", lambda d: log_msg(pair_id, f"Account event: {str(d)[:80]}"))
    conn.on_open(lambda: [
        log_msg(pair_id, "✅ WebSocket verbunden"),
        conn.send("SubscribeAccounts", []),
        conn.send("SubscribeOrders", [int(s["tsxAccountId"])]),
        conn.send("SubscribePositions", [int(s["tsxAccountId"])]),
    ])
