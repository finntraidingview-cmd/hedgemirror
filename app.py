...
MA_BASE  = "https://mt-client-api-v1.agiliumtrade.agiliumtrade.ai"
...
def ma_proxy(path):
    ...
    url = f"{MA_BASE}/users/current/accounts/{account_id}" if account_id and path == "account" else (f"{MA_BASE}/users/current/accounts/{account_id}/{path}" if account_id else f"{MA_BASE}/users/current/{path}")
    try:
        r = requests.request(request.method, url,
            json=request.get_json(silent=True), headers=h, timeout=15)
